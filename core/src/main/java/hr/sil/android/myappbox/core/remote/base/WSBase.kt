...
package hr.sil.android.myappbox.core.remote.base

import android.util.Base64
import hr.sil.android.myappbox.core.remote.model.REncryptRequest
import hr.sil.android.myappbox.core.remote.model.REncryptResponse
...
abstract class WSBase {
    ...
    private var onUnauthorizedListener: () -> Unit = { }
    fun registerOnUnauthorizedListener(listener: () -> Unit) {
    ...
    private suspend fun <T> wrapAwaitDataAndHttpStatusResponse(call: Call<T>, methodName: String): Response<T>? {
        return try {
            val response = call.awaitResponse()
            if (!response.isSuccessful) {
                log.error("Error in $methodName! Code=${response.code()}")
    ...
    protected suspend fun <T> wrapAwaitDataAndHttpStatus(call: Call<T>, methodName: String, defaultNullValue: T? = null): Response<T>? {
        val response = wrapAwaitDataAndHttpStatusResponse(call, methodName)
        log.info("Response: ${methodName} ${response?.code()} ${response?.errorBody().toString()}")
        return response
    }
